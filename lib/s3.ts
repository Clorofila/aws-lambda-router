...
export const process: ProcessMethod<S3Config, S3Event, Context, any> = (s3Config, event, context) => {
    ...
    for (const record of event.Records) {
        for (const routeConfig of s3Config.routes) {
            ...
            if (s3Config.debug) {
                console.log(`match record '${record.eventName}'/'${record.s3.bucket.name}'/'${
                    record.s3.object.key
                }': bucketMatch '
                  ${bucketNameMatched}', eventMatch '${eventNameMatched}', key '${objectKeyPrefixMatched}' for route '${JSON.stringify(
                      routeConfig,
                  )}'`);
            }
