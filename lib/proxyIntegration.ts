...
) => Promise<APIGatewayProxyStructuredResultV2 | void> | APIGatewayProxyStructuredResultV2 | void;
export type ProxyIntegrationEvent<T = unknown> = Omit<APIGatewayProxyEventV2, 'body'> &
...
> = (proxyIntegrationConfig, event, context) => {
    ...
    const headers: APIGatewayProxyStructuredResultV2['headers'] = proxyIntegrationConfig.cors
    ...
    if (proxyIntegrationConfig.proxyPath) {
        event.requestContext.http.path = (event.pathParameters || {})[proxyIntegrationConfig.proxyPath];
        if (proxyIntegrationConfig.debug) {
            console.log(`proxy path is set: ${proxyIntegrationConfig.proxyPath}`);
            console.log(`proxy path with event path: ${event.requestContext.http.path}`);
        }
    } else {
        event.requestContext.http.path = normalizeRequestPath(event, proxyIntegrationConfig.removeBasePath);
    }
...
const normalizeRequestPath = (event: APIGatewayProxyEventV2, removeBasePath: boolean = true): string => {
    ...
    if (
        ...
        const groups = /\/[^\/]+(.*)/.exec(event.requestContext.http.path) || [null, null];
        return groups[1] || '/';
    }
