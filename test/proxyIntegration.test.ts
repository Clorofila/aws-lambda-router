...
describe('proxyIntegration.routeHandler.selection', () => {
    it('should select longer match without context (for backward compatibility)', () => {
        ...
        proxyIntegration(
            ...
            { httpMethod: 'GET', path: '/123' } as APIGatewayProxyEvent,
            context,
        );
        expect(actionSpy).toHaveBeenCalledWith(
            { httpMethod: 'GET', path: '/123', paths: {}, routePath: '/123' },
            jasmine.anything(),
        );
    });
    ...
    forEach([['/:param'], ['/{param}']]).it('should select parameter match', path => {
        const actionSpy = jasmine.createSpy('action');
        proxyIntegration(
            {
                routes: [
            ...
            { httpMethod: 'GET', path: '/456' } as APIGatewayProxyEvent,
            context,
        );
        expect(actionSpy).toHaveBeenCalledWith(
            { httpMethod: 'GET', path: '/456', routePath: path, paths: { param: '456' } },
            context,
        );
    });
    forEach([['/:param'], ['/{param}']]).it('should select static match', path => {
        const actionSpy = jasmine.createSpy('action');
        proxyIntegration(
            {
                routes: [
        ...
        expect(actionSpy).toHaveBeenCalledWith(
            { httpMethod: 'GET', path: '/123', routePath: '/123', paths: {} },
            context,
        );
    });
    forEach([['/:param'], ['/{param}']]).it('should match urlencoded path', path => {
        const actionSpy = jasmine.createSpy('action');
        proxyIntegration(
            {
                routes: [
            ...
            { httpMethod: 'GET', path: '/%2Fwirtschaft%2Farticle85883...tml' } as APIGatewayProxyEvent,
            context,
        );
        expect(actionSpy).toHaveBeenCalledWith(
            {
    ...
    forEach([['/:param'], ['/{param}']]).it('should select match containing hyphen', path => {
        const actionSpy = jasmine.createSpy('action');
        proxyIntegration(
            {
                routes: [{ path: path, method: 'GET', action: actionSpy }],
            },
            { httpMethod: 'GET', path: '/%2Fdeutschland-bewegt-sich%2F' } as APIGatewayProxyEvent,
            context,
        );
        expect(actionSpy).toHaveBeenCalledWith(
            {
    ...
    forEach([['/:param'], ['/{param}']]).it('should select match containing question marks and dots', path => {
        const actionSpy = jasmine.createSpy('action');
        proxyIntegration(
            {
                routes: [{ path: path, method: 'GET', action: actionSpy }],
    ...
    it('should add cors headers to OPTIONS request', async () => {
        const result = await proxyIntegration(
            ...
            { httpMethod: 'OPTIONS', path: '/' } as APIGatewayProxyEvent,
            context,
        );
    ...
    it('should add cors headers to GET request', async () => {
        const result = await proxyIntegration(
            ...
            { httpMethod: 'GET', path: '/' } as APIGatewayProxyEvent,
            context,
        );
...
describe('proxyIntegration.routeHandler', () => {
    ...
    it('should remove basepath from multi-slash-path if coming over custom domain name', () => {
        ...
        const event = {
            httpMethod: 'GET',
            path: '/shortcut-itemsdev/123/456',
            headers: { Host: 'api.ep.welt.de' },
            requestContext: { apiId: 'blabla' },
        };
        proxyIntegration(
    ...
    it('should return 400 for an invalid body', async () => {
        const result = await proxyIntegration(
            { routes: [{} as any] },
            { httpMethod: 'GET', path: '/', body: '{keinJson' } as APIGatewayProxyEvent,
            context,
        );
        expect(result).toEqual({
            statusCode: 400,
    ...
    forEach([
        ...
        const result = await proxyIntegration(
            routeConfig,
            { path, httpMethod: method } as APIGatewayProxyEvent,
            context,
        );
        expect(result).toEqual({
            statusCode: 200,
    ...
    forEach([
        ...
        expect(spiedAction).toHaveBeenCalledWith(
            { path, httpMethod: 'GET', paths: expectedPathValues, routePath: pathConfig },
            context,
        );
    });
            ...
            routes: [
                {
                    method: 'GET',
                    path: '/',
                    action: () => Promise.reject({ reason: 'myError', message: 'doof' }) as any,
                },
            ],
